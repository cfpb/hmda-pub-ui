...
RUN yum install -y epel-release && \
    ...
    mv nginx/* /etc/nginx && \
    ls -d -1 * | grep -v '^\(data-publication\|docker-entrypoint.sh\|env.sh\)$' | xargs rm -rf && \
    touch /run/nginx.pid && \
    chown -R $NGINX_USER:$NGINX_USER data-publication/js/app.min.js /etc/nginx /run/nginx.pid
